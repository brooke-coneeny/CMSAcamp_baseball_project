...
- Exit velocties were sampled to create a mock data set representing this player's balls hit into play due to their new attack angle
...
```{r}
#Slide for Adam's way and the issues there (doens't include pitch information)
#Slide (or more) for our final way in adjusting launch angle + graphs 
#Slide for Erin's EDA
#Final slides to wrap up other considerations (swinging differently means what you're pitched changes,
  #what are the mental implications of changing a hitter's swing, if the woba change isn't "worth it"
  #there might not be a reason to change the hitter's swing)
#fix mental game image and update steps to include ^^ that stuff
```
